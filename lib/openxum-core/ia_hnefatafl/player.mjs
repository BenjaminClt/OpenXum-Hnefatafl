"use strict";
import Color from '../games/hnefatafl/color.mjs';
import Coordinates from '../games/hnefatafl/coordinates.mjs';
class Player {
  ...
  move() {
    ...
    for (let coup = 0; coup < possibleMoves.length; coup++) {
      ...
      e.move(move);
      let tmpMax = -this._negamax(e, this._depth, -900, 900, (this._color === Color.BLACK ? Color.WHITE : Color.BLACK));
  ...
  _evaluate(b, c, depth) {
    ...
    if (c === Color.BLACK) {
      score += - 0.5 * shortestDist + 2 * countPawnKing;
    } else {
      score += 1.5 * shortestDist - 0.5 * countPawnKing;
    }
